...
这是一个完整的解决方案，让 `browser-use` 可以在本地通过 Chrome DevTools Protocol (CDP) 连接到运行在 PPIO sandbox 中的远程 Chromium 实例。
...
```
本地 browser-use → PPIO Sandbox 外部域名:9223 → Go反向代理 → Chromium:9222
```
...
### 2. 连接建立阶段  
- `browser-use` 向 PPIO Sandbox 外部域名发起连接请求
...
**关键重写点：**
- `ws://` → `wss://` (PPIO Sandbox HTTPS 环境要求安全 WebSocket)
...
- PPIO Sandbox 通过 HTTPS 对外提供服务，需要 WSS 协议
...
构建脚本会自动：
1. 检查 Go 和 PPIO Sandbox CLI 依赖
2. 编译 Go 反向代理为 Linux x86 二进制文件  
3. 构建 PPIO Sandbox Template
